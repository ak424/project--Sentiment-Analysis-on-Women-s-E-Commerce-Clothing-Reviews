...
st.title("Sentiment Analysis of Women's E-Commerce Clothing Reviews")
...
def load_data():
    df=pd.read_csv("/content/drive/My Drive/Classroom/ML-07B7/clothing_review.csv")
   
    return df

df = load_data()
#dropping the unnecessary colmns from the datasets,
df=df.drop(['Clothing ID','Age','Title','Recommended IND','Positive Feedback Count','Division Name','Department Name','Class Name','Unnamed: 0'],axis=1)
df = df.rename(columns = {'Review Text': 'Text', 'Rating': 'Score'})
...
text_model = Pipeline([('tfidf',TfidfVectorizer()),('model',SVC(kernel='linear'))])
